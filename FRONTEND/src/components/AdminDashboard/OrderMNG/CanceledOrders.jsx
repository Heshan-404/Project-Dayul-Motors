...
export default function CanceledOrders({ orders = [], searchValue }) {
  const [rows, setRows] = useState([]);

  useEffect(() => {
    // Transforming orders data to the table row format
    const transformedRows = orders
      .filter((order) => order.orderstatus === "Canceled") // Filter only canceled orders
      .map((order, index) => ({
        id: index + 1,
